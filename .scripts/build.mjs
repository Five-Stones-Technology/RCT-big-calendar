...
await $`cross-env NODE_OPTIONS=--openssl-legacy-provider NODE_ENV=production rollup -c`;
console.log(chalk.blue(`Compiling 'lib' js files`))
// build files used for overrides
await $`cross-env NODE_OPTIONS=--openssl-legacy-provider NODE_ENV=production RBC_CJS_BUILD=true babel src --out-dir lib`
...
await $`sass ./lib/sass/styles.scss ./lib/css/fs-big-calendar.css`
